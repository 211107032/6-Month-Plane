...
st.set_page_config(page_title="Peacock Feather Image Processor", layout="wide")

# Title
st.title("Peacock Feather - Multi-Color Channel Visualizer")

# Load image from URL
@st.cache_data
def load_image():
    url = "https://m.media-amazon.com/images/I/81JSw5mE54L._UF894,1000_QL80_.jpg"
    response = requests.get(url)
    return Image.open(BytesIO(response.content)).convert("RGB")

# Load and display image
Peacock = load_image()
st.image(Peacock, caption="Original Peacock Feather Image", use_container_width=True)
